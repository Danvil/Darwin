...
namespace Collision
{
	...
	bool CellRay(const Ci& c_cell, const Vec3f& ray_a, const Vec3f& ray_u, Tester test, Ci& cc_world, float& distance, int& side) {
		float d_start;
		Vec3f cell_min = Common::CellMin(c_cell);
		Vec3f cell_max = Common::CellMax(c_cell);
		// find start point
		...
		float lambda;
		bool hit = GridRayIntersect(start, ray_u, Ci(0,0,0), Ci(Common::CellSize,Common::CellSize,Common::CellSize), test, result, lambda, side);
		if(hit) {
			cc_world = Common::LocalToWorld(c_cell, result);
	...
	struct IsSolidTester
	{
		...
		bool operator()(int x, int y, int z, float lambda) {
			assert(0 <= x && x < int(Common::CellSize));
			assert(0 <= y && y < int(Common::CellSize));
			assert(0 <= z && z < int(Common::CellSize));
